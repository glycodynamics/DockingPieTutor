...
Open the GlycoTorch Vina output files, ```Run_1_GTVina.pdbqt```, in PyMOL and visualize the docking poses generated by Vina and GlycoTorch. The top-ranked pose (M1) from GlycoTorch closely resembles the crystal structure binding mode of GAG, while only the 5th ranked docking pose from Vina is in close agreement with the crystal structure binding mode.
